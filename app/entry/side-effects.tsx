...
const BG='#F0EAE4', TERRACOTTA='#C4784B', DARK='#1C0F09';
const SHADOW={shadowColor:DARK,shadowOffset:{width:0,height:8} as const,shadowOpacity:0.12,shadowRadius:24,elevation:8};
...
function GB({r=24}:{r?:number}){
  return <View pointerEvents="none" style={{position:'absolute',top:0,left:0,right:0,bottom:0,borderRadius:r,borderWidth:1,borderTopColor:'rgba(255,255,255,0.72)',borderLeftColor:'rgba(255,255,255,0.48)',borderRightColor:'rgba(255,255,255,0.14)',borderBottomColor:'rgba(255,255,255,0.08)'}}/>;
}
  ...
  return(
    <View style={{borderRadius:24,marginTop:mt,...SHADOW}}>
      <View style={{borderRadius:24,overflow:'hidden'}}>
        ...
        <View style={[StyleSheet.absoluteFillObject,{borderRadius:24,backgroundColor:'rgba(255,255,255,0.35)'}]}/>
        <GB r={24}/>
...
const SL=({t}:{t:string})=><Text style={{fontSize:11,fontWeight:'700',color:'rgba(28,15,9,0.5)',letterSpacing:3.5,textTransform:'uppercase',marginBottom:16}}>{t}</Text>;
    ...
    <KeyboardAvoidingView style={{flex:1,backgroundColor:BG}} behavior={Platform.OS==='ios'?'padding':'height'}>
      <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:20,paddingTop:insets.top+10,paddingBottom:14,backgroundColor:BG}}>
        <TouchableOpacity style={{width:40,height:40,borderRadius:20,overflow:'hidden',alignItems:'center',justifyContent:'center',...SHADOW,shadowOpacity:0.08,shadowRadius:12}} onPress={()=>router.back()} activeOpacity={0.7}>
          ...
          <View style={[StyleSheet.absoluteFillObject,{borderRadius:20,backgroundColor:'rgba(255,255,255,0.3)'}]}/>
          <GB r={20}/><Ionicons name="chevron-back" size={22} color={DARK}/>
        </TouchableOpacity>
      ...
      <ScrollView style={{flex:1}} contentContainerStyle={{paddingHorizontal:20,paddingTop:4,paddingBottom:insets.bottom+100}} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            {SYMPTOMS.map(({label,value})=>{
              ...
              return(
                <TouchableOpacity key={value} onPress={()=>toggle(value)} activeOpacity={0.75}
                  style={{borderRadius:20,overflow:'hidden',paddingVertical:10,paddingHorizontal:16,...SHADOW,shadowOpacity:0.07,shadowRadius:8,shadowOffset:{width:0,height:3},shadowColor:a?TERRACOTTA:DARK}}>
                  {a?<View style={[StyleSheet.absoluteFillObject,{borderRadius:20,backgroundColor:'rgba(196,120,75,0.9)'}]}/>
                    :<><BlurView intensity={60} tint="light" style={StyleSheet.absoluteFillObject}/><View style={[StyleSheet.absoluteFillObject,{borderRadius:20,backgroundColor:'rgba(255,255,255,0.5)'}]}/></>}
                  <GB r={20}/><Text style={{fontSize:14,fontWeight:'600',color:a?'#FFF':DARK}}>{label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
              ...
              {Array.from({length:10},(_,i)=>i+1).map(l=>{
                ...
                return(
                  <TouchableOpacity key={l} onPress={()=>setSev(l)} activeOpacity={0.75}
                    ...
                    <Text style={{fontSize:13,fontWeight:'800',color:a?'#FFF':DARK}}>{l}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            <View style={{flexDirection:'row',justifyContent:'space-between'}}>
              {['Mild','Moderate','Severe'].map(l=><Text key={l} style={{fontSize:10,fontWeight:'600',color:'rgba(28,15,9,0.4)',textTransform:'uppercase',letterSpacing:0.5}}>{l}</Text>)}
            </View>
          </Card>
          <Card mt={16}>
            <SL t="CURRENT PHASE"/>
        ...
        <Card mt={16}>
          ...
          <View style={{borderRadius:16,overflow:'hidden',minHeight:90}}>
            ...
            <View style={[StyleSheet.absoluteFillObject,{borderRadius:16,backgroundColor:'rgba(255,255,255,0.3)'}]}/>
            <GB r={16}/>
            <TextInput style={{fontSize:15,color:DARK,padding:14,minHeight:90,lineHeight:22}} placeholder="Describe how you're feeling…" placeholderTextColor="rgba(28,15,9,0.35)" value={notes} onChangeText={setNotes} multiline numberOfLines={3} textAlignVertical="top" blurOnSubmit/>
          </View>
        </Card>
      </ScrollView>
      <View style={{paddingHorizontal:20,paddingTop:12,paddingBottom:insets.bottom+16,backgroundColor:BG,borderTopWidth:1,borderTopColor:'rgba(28,15,9,0.06)'}}>
        <TouchableOpacity style={{backgroundColor:has?TERRACOTTA:'rgba(196,120,75,0.3)',borderRadius:28,paddingVertical:17,alignItems:'center',justifyContent:'center',shadowColor:TERRACOTTA,shadowOffset:{width:0,height:8},shadowOpacity:has?0.35:0,shadowRadius:20,elevation:has?10:0}} onPress={save} activeOpacity={has?0.8:1} disabled={!has||loading}>
          {loading?<ActivityIndicator color="#FFF" size="small"/>:<Text style={{fontSize:16,fontWeight:'800',color:has?'#FFF':'rgba(28,15,9,0.35)',letterSpacing:0.4}}>Log Side Effects</Text>}
        </TouchableOpacity>
      </View>
