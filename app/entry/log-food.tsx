...
export default function LogFoodScreen() {
  ...
  function handleQueryChange(text: string) {
    ...
    if (!text.trim()) { setSearchResults([]); setSuggestions([]); return; }
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true);
      const foodResults = await searchUSDA(text);
      let r = foodResults;
      if (r.length === 0) {
  ...
  function handleSuggestionTap(suggestion: string) {
    ...
    if (autocompleteRef.current) clearTimeout(autocompleteRef.current);
    setSearching(true);
    searchUSDA(suggestion).then(async (foodResults) => {
      let r = foodResults;
      if (r.length === 0) {
  ...
  async function handleParse() {
    ...
    setDescribeError('');
    try {
      const parsedDishes = await parseDescriptionToDishes(describeText.trim());
      // Dispatch to background processing and navigate away
      startFoodTask({ source: 'describe', parsedDishes });
      router.back();
    } catch (err) {
      setDescribeError(describeErrorMessage(err));
      setDescribing(false);
    }
  }
            ...
            <View style={s.trayActions}>
              {trayItems.length >= 2 && !showSaveInput && (
                <TouchableOpacity style={s.traySaveBtn} onPress={() => setShowSaveInput(true)} activeOpacity={0.8}>
                  <Text style={s.traySaveBtnText}>Save as Meal</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
