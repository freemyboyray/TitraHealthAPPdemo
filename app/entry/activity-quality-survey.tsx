...
const QUESTIONS = [
  ...
  'I included resistance or strength training at least once.',
  'I hit my daily step goals on most days.',
  'I had enough energy to move and exercise the way I wanted.',
  'Physical activity felt manageable given how I\'ve been feeling.',
] as const;
...
function scoreInterpretation(score: number): { label: string; color: string } {
  if (score >= 15) return { label: 'High',      color: '#27AE60' };
  if (score >= 9)  return { label: 'Moderate',  color: '#F6CB45' };
  if (score >= 4)  return { label: 'Low',        color: '#E8960C' };
...
export default function ActivityQualitySurveyScreen() {
  ...
  // Direct scale: high raw = high activity (good)
  const score100 = Math.round((score / 20) * 100);
  ...
  async function handleSave() {
    ...
    try {
      ...
      await addWeeklyCheckin('activity_quality', answersMap, score100);
      router.replace({
        pathname: '/entry/checkin-summary',
        params: {
          type: 'activity_quality',
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontSize: 18, fontWeight: '800', color: colors.textPrimary }}>Activity & Strength Check-In</Text>
      ...
      >
        ...
        <Text style={{ fontSize: 12, color: colors.textMuted, textAlign: 'center', marginTop: 8, lineHeight: 17 }}>
          Answer for this past week. Activity scores help preserve lean mass during GLP-1 treatment.
        </Text>
      </ScrollView>
