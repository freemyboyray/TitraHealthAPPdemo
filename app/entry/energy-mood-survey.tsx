...
const QUESTIONS = [
  'My energy level has been consistent throughout the day this week.',
  'I have felt mentally clear and focused.',
  'My mood has been stable with fewer lows or irritability.',
  'I have had enough motivation to stick to my daily routine.',
  'I woke up feeling rested most mornings.',
] as const;

const DOT_LABELS = ['Not at all', 'A little', 'Sometimes', 'Often', 'Extremely'];

function scoreToHundred(raw: number): number {
  // raw 0–20, normalize to 0–100
  return Math.round((raw / 20) * 100);
}

function scoreInterpretation(score: number): { label: string; color: string } {
  if (score >= 15) return { label: 'Strong',   color: '#27AE60' };
  if (score >= 10) return { label: 'Moderate', color: '#F6CB45' };
  if (score >= 5)  return { label: 'Low',      color: '#E8960C' };
...
export default function EnergyMoodSurveyScreen() {
  ...
  const score100 = scoreToHundred(rawScore);
  const interpretation = scoreInterpretation(rawScore);
  ...
  async function handleSave() {
    ...
    try {
      ...
      await addWeeklyCheckin('energy_mood', answersMap, score100);
      router.replace({
