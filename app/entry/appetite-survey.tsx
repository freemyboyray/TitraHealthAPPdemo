...
function scoreInterpretation(score: number): { label: string; color: string } {
  if (score >= 15) return { label: 'Well Controlled', color: '#27AE60' };
  if (score >= 10) return { label: 'Moderate',        color: '#F6CB45' };
  if (score >= 5)  return { label: 'Mild Control',    color: '#E8960C' };
  return              { label: 'Low Control',     color: '#E53E3E' };
}
...
export default function AppetiteSurveyScreen() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await addWeeklyCheckin('appetite', answersMap, score100);
      router.replace({
    ...
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      >
        ...
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontSize: 18, fontWeight: '800', color: '#FFFFFF' }}>Appetite &amp; Satiety</Text>
