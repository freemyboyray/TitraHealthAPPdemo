...
const BG = '#F0EAE4', TERRACOTTA = '#C4784B', DARK = '#1C0F09', WHITE = '#FFFFFF';
...
function GlassBorder({ r = 28 }: { r?: number }) {
  return <View pointerEvents="none" style={{ position:'absolute',top:0,left:0,right:0,bottom:0,borderRadius:r,borderWidth:1,borderTopColor:'rgba(255,255,255,0.65)',borderLeftColor:'rgba(255,255,255,0.42)',borderRightColor:'rgba(255,255,255,0.14)',borderBottomColor:'rgba(255,255,255,0.08)' }} />;
}
function StepBtn({ icon, onPress, disabled }: { icon:'add'|'remove'; onPress:()=>void; disabled:boolean }) {
  return (
    <TouchableOpacity onPress={onPress} disabled={disabled} activeOpacity={0.7} style={[s.stepBtn, disabled&&{opacity:0.35}]}>
      ...
      <View style={[StyleSheet.absoluteFillObject,{borderRadius:32,backgroundColor:'rgba(255,255,255,0.3)'}]}/>
      <GlassBorder r={32}/>
    ...
    <KeyboardAvoidingView style={{flex:1,backgroundColor:BG}} behavior={Platform.OS==='ios'?'padding':'height'}>
      ...
      <View style={s.header}>
        <TouchableOpacity onPress={()=>router.back()} activeOpacity={0.7} style={s.back}>
          <BlurView intensity={75} tint="light" style={StyleSheet.absoluteFillObject}/>
          <View style={[StyleSheet.absoluteFillObject,{borderRadius:22,backgroundColor:'rgba(255,255,255,0.3)'}]}/>
          <GlassBorder r={22}/><Ionicons name="chevron-back" size={22} color={DARK}/>
        </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={[s.scroll,{paddingBottom:insets.bottom+32}]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.card,{marginBottom:16}]}>
          <View style={{borderRadius:28,overflow:'hidden'}}>
            ...
            <View style={[StyleSheet.absoluteFillObject,{borderRadius:28,backgroundColor:'rgba(255,255,255,0.35)'}]}/>
            <GlassBorder r={28}/>
            <View style={{alignItems:'center',paddingHorizontal:24,paddingVertical:28}}>
              <Text style={{fontSize:11,fontWeight:'700',color:TERRACOTTA,letterSpacing:3.5,textTransform:'uppercase',marginBottom:22}}>TODAY</Text>
              ...
              <View style={{flexDirection:'row',backgroundColor:'rgba(28,15,9,0.07)',borderRadius:32,padding:4,marginBottom:14}}>
                {(['lbs','kg'] as Unit[]).map(u=>(
                  <TouchableOpacity key={u} onPress={()=>setUnit(u)} activeOpacity={0.8}
                    style={[{paddingHorizontal:26,paddingVertical:9,borderRadius:28},unit===u&&{backgroundColor:TERRACOTTA}]}>
                    <Text style={{fontSize:13,fontWeight:'700',letterSpacing:2,color:unit===u?WHITE:'rgba(28,15,9,0.45)'}}>{u.toUpperCase()}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <Text style={{fontSize:11,color:'rgba(28,15,9,0.32)',letterSpacing:1.5,textTransform:'uppercase'}}>{mn} – {mx} {unit.toUpperCase()}</Text>
            </View>
          </View>
        </View>
        <View style={[s.card,{marginBottom:24}]}>
          <View style={{borderRadius:24,overflow:'hidden'}}>
            ...
            <View style={{flexDirection:'row',alignItems:'center',paddingHorizontal:18,paddingVertical:16}}>
              <Ionicons name="create-outline" size={18} color="rgba(28,15,9,0.42)" style={{marginRight:10}}/>
              <TextInput style={{flex:1,fontSize:16,fontWeight:'500',color:DARK}} placeholder="Add a note… (optional)" placeholderTextColor="rgba(28,15,9,0.35)" value={notes} onChangeText={setNotes} returnKeyType="done" maxLength={200}/>
            </View>
          </View>
        </View>
        <TouchableOpacity onPress={save} activeOpacity={0.85} disabled={loading} style={[s.saveBtn,loading&&{opacity:0.75}]}>
          {loading ? <ActivityIndicator color={WHITE} size="small"/> : <Text style={{fontSize:17,fontWeight:'700',color:WHITE,letterSpacing:0.5}}>Save</Text>}
        </TouchableOpacity>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  date:{fontSize:11,fontWeight:'600',color:'rgba(28,15,9,0.45)',textAlign:'center',letterSpacing:3.5,marginBottom:20},
  ...
  saveBtn:{height:56,borderRadius:28,backgroundColor:TERRACOTTA,alignItems:'center',justifyContent:'center',shadowColor:TERRACOTTA,shadowOffset:{width:0,height:8},shadowOpacity:0.4,shadowRadius:20,elevation:8},
});
