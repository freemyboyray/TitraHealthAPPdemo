...
const QUESTIONS = [
  'I felt down, depressed, or hopeless this week.',
  'I had little interest or pleasure in things I usually enjoy.',
  'I felt nervous, anxious, or on edge.',
  'I could not stop or control worrying.',
  'Mood or anxiety affected my eating habits or medication adherence.',
] as const;
...
export default function MentalHealthSurveyScreen() {
  ...
  // Inverted: low raw = stable mood = high score
  const score100 = Math.round((1 - score / 20) * 100);
  const showProviderPrompt = score >= 15;
  ...
  async function handleSave() {
    ...
    try {
      ...
      await addWeeklyCheckin('mental_health', answersMap, score100);
      router.replace({
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontSize: 20, fontWeight: '800', color: colors.textPrimary }}>Mental Health Check-In</Text>
      ...
      >
        {/* Provider prompt banner - shown when raw score ≥ 15 */}
        {showProviderPrompt && (
          <View
            style={{
              borderRadius: 16, padding: 14, marginBottom: 12,
              backgroundColor: 'rgba(246,203,69,0.15)',
              borderWidth: 1, borderColor: 'rgba(246,203,69,0.4)',
              flexDirection: 'row', alignItems: 'center', gap: 10,
            }}
          >
            <Ionicons name="alert-circle" size={20} color="#F6CB45" />
            <Text style={{ flex: 1, fontSize: 15, color: colors.textPrimary, lineHeight: 18 }}>
        ...
        <Text style={{ fontSize: 13, color: colors.textMuted, textAlign: 'center', marginTop: 8, lineHeight: 16 }}>
          Questions adapted from PHQ-2 and GAD-2 screening tools. This is not a clinical diagnosis.
        </Text>
      </ScrollView>
