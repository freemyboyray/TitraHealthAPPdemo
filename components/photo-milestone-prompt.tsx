...
export function PhotoMilestonePrompt({ lbs, achievement, onTakePhoto, onDismiss }: Props) {
  ...
  const accent = achievement
    ? ACHIEVEMENT_ACCENT[achievement.category]
    : ACHIEVEMENT_ACCENT.weight;

  const iconName = achievement ? (achievement.icon as any) : 'camera';
