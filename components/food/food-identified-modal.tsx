...
// Success green — distinguishes a confirmed match from the orange "no match" ✕.
...
type Props = {
  ...
  colors: AppColors;
  isDark: boolean;
  /** Override the title (defaults to "Review complete"). */
  title?: string;
  /** Override the body copy. */
  message?: string;
  /** Primary CTA label (defaults to "Review meal"). */
  primaryLabel?: string;
  onPrimary: () => void;
  onDismiss: () => void;
};

// Success sibling of FoodNotIdentifiedModal — same sheet/grid/tile design, with a
// green ✓ badge in place of the orange ✕. Shown when AI photo analysis succeeds so
// the flow "completes" instead of silently dropping you onto the review cards.
export function FoodIdentifiedModal({
  visible,
  colors,
  isDark,
  title = 'Review complete',
  message = 'We identified your meal. Check the details below, then add it to your log.',
  primaryLabel = 'Review meal',
  onPrimary,
  onDismiss,
}: Props) {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onDismiss} statusBarTranslucent>
      <View style={[s.root, { backgroundColor: scrim }]}>
        ...
        <View style={[s.sheet, { backgroundColor: cardBg, borderColor: colors.borderSubtle, paddingBottom: insets.bottom + 18 }]}>
          ...
          {/* ── Decorative header: faded food-icon grid + the "matched" graphic ── */}
          <View style={s.headerArt}>
            ...
            <View style={s.graphicRow} pointerEvents="none">
              ...
              <View style={[s.checkBadge, { backgroundColor: SUCCESS, borderColor: cardBg }]}>
