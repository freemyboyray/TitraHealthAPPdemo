...
/**
 * Estimate a user's percentile rank given their weight-loss % and the cohort's
 * p25 / p50 / p75 values. Returns 0–100.
 */
export function computePeerPercentile(
  userWeightLossPct: number,
  p25: number,
  p50: number,
  p75: number,
): number {
  if (userWeightLossPct <= p25) {
    // Below or at 25th percentile — scale 0-25
    return p25 > 0
      ? Math.max(0, Math.round((userWeightLossPct / p25) * 25))
      : 0;
  }
  if (userWeightLossPct <= p50) {
    // Between 25th and 50th
    const range = p50 - p25;
    return range > 0
      ? Math.round(25 + ((userWeightLossPct - p25) / range) * 25)
      : 37;
  }
  if (userWeightLossPct <= p75) {
    // Between 50th and 75th
    const range = p75 - p50;
    return range > 0
      ? Math.round(50 + ((userWeightLossPct - p50) / range) * 25)
      : 62;
  }
  // Above 75th — extrapolate 75-100
  const iqr = p75 - p25;
  if (iqr <= 0) return 90;
  const above = (userWeightLossPct - p75) / iqr;
  return Math.min(99, Math.round(75 + above * 25));
}
