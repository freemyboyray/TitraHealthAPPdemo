...
How we changed permission handling: Previously, AI processing was enabled as part of accepting the terms. We have removed that. The app now obtains explicit, separate permission before sending any data, in two places. First, during onboarding, a new dedicated "Enable AI features?" screen explains exactly what is sent and to whom, and requires the user to actively tap "Allow." It now defaults to off, and declining is non-blocking, so all core tracking features work without AI. Second, the first time a user taps any AI feature, a new consent dialog naming OpenAI appears, and the request only proceeds if the user taps "Allow."

No data is sent to OpenAI unless the user has granted this consent. Consent can be reviewed or revoked at any time in Settings, under Privacy and Data.
